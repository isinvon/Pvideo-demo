<template> 
  ...
  .disclaimer-title {
    ...
    opacity: 0;
    visibility: hidden;
    transform: scaleY(0); /* 初始状态：元素不可见，缩放为0 */
    transform-origin: top; /* 缩放动画从顶部开始 */
    transition: opacity 0.5s ease, visibility 0s 0.5s, transform 0.5s ease; /* 过渡动画 */
  }
  ...
  .el-card:hover .disclaimer-divider {
    opacity: 1;
    visibility: visible;
    transform: scaleY(1); /* 恢复正常尺寸 */
    transition: opacity 0.5s ease, visibility 0s 0s, transform 0.5s ease; /* 悬浮时的过渡动画 */
  }
  </style>
